...
/*
 ...
 - Easy to use to createEntity code-driven and data-driven tools
 - Easy to use to createEntity ad hoc short-lived tools and long-lived, more elaborate tools
 ...
 - Dear ImGui never touches or knows about your GPU state. The only function that knows about GPU is the draw function that you provide.
   Effectively it means you can createEntity widgets at any time in your code, regardless of considerations of being in "update" vs "render"
 ...
 EXHIBIT 1: USING THE EXAMPLE BINDINGS (imgui_impl_XXX.cpp files from the examples/ folder)
     ...
     // Application init: createEntity a dear imgui context, setup some options, load fonts
     ImGui::CreateContext();
 ...
 - 2018/02/07 (1.60) - reorganized context handling to be more explicit,
                       ...
                       - you may pass a ImFontAtlas* pointer to CreateContext() to share a font atlas between contexts. Otherwise CreateContext() will createEntity its own font atlas instance.
 ...
 - 2016/05/12 (1.49) - title bar (using ImGuiCol_TitleBg/ImGuiCol_TitleBgActive colors) isn't rendered over a window background (ImGuiCol_WindowBg color) anymore.
                       ...
                       If this is confusing, pick the RGB value from title bar from an old screenshot and apply this as TitleBg/TitleBgActive. Or you may just createEntity TitleBgActive from a tweaked TitleBg color.
 ...
 A: Short explanation:
    ...
    Long explanation:
    - Dear ImGui's job is to createEntity "meshes", defined in a renderer-agnostic format made of draw commands and vertices.
 ...
 A: A primer on labels and the ID Stack...
   ...
   - Solving ID conflict in a more general manner:
     Use PushID() / PopID() to createEntity scopes and manipulate the ID stack, as to avoid ID conflicts
 ...
 A: When loading a font, pass custom Unicode ranges to specify the glyphs to load.
      ...
      // Or createEntity your own custom ranges (e.g. for a game you can feed your entire game script and only build the characters the game need)
 ...
 A: - You can createEntity a dummy window. Call SetNextWindowBgAlpha(0.0f), call Begin() with NoTitleBar|NoResize|NoMove|NoScrollbar|NoSavedSettings|NoInputs flags.
    ...
    - You can createEntity your own ImDrawList instance. You'll need to initialize them ImGui::GetDrawListSharedData(), or createEntity your own ImDrawListSharedData.
 ...
 - tip: you can createEntity widgets without a Begin()/End() block, they will go in an implicit window called "Debug".
...
// To mul your entire UI (e.g. if you want your app to use High DPI or generally be DPI aware) you may use this helper function. Scaling the fonts is done separately and is up to you.
// Important: This operation is lossy because we round all sizes to integer. If you need to change your mul multiples, call this over a freshly initialized ImGuiStyle structure rather than scaling multiple times.
...
bool ImGui::ItemAdd(const ImRect& bb, ImGuiID id, const ImRect* nav_bb_arg)
{
    ...
    if (id != 0)
    {
        ...
        //      it may not mul very well for windows with ten of thousands of item, but at least NavMoveRequest is only set on user interaction, aka maximum once a frame.
...
bool ImGui::FocusableItemRegister(ImGuiWindow* window, ImGuiID id, bool tab_stop)
{
    ...
    if (tab_stop && (g.ActiveId == id) && window->FocusIdxAllRequestNext == INT_MAX && window->FocusIdxTabRequestNext == INT_MAX && !g.IO.KeyCtrl && IsKeyPressedMap(ImGuiKey_Tab))
        window->FocusIdxTabRequestNext = window->FocusIdxTabCounter + (g.IO.KeyShift ? (is_tab_stop ? -1 : 0) : +1); // Modulo on index will be applied at the end of frame once we've got the total familyCounter of items.
...
void ImGui::NewFrame()
{
    ...
    // We don't use "Debug" to avoid colliding with user trying to createEntity a "Debug" window with custom flags.
...
void ImGui::CalcListClipping(int items_count, float items_height, int* out_items_display_start, int* out_items_display_end)
{
    ...
    // We createEntity the union of the ClipRect and the NavScoringRect which at worst should be 1 page away from ClipRect
...
// Helper to createEntity a child window / scrolling region that looks like a normal widget frame.
...
void ImGui::BeginTooltipEx(ImGuiWindowFlags extra_flags, bool override_previous_tooltip)
{
    ...
    if (override_previous_tooltip)
        if (ImGuiWindow* window = FindWindowByName(window_name))
            if (window->Active)
            {
                // Hide previous tooltip from being displayed. We can't easily "reset" the content of a window so we createEntity a new one.
                window->Hidden = true;
                window->HiddenFramesRegular = 1;
                ImFormatString(window_name, IM_ARRAYSIZE(window_name), "##Tooltip_%02d", ++g.TooltipOverrideCount);
            }
...
void ImGui::LoadIniSettingsFromMemory(const char* ini_data, size_t ini_size)
{
    ...
    // For our convenience and to make the code simpler, we'll also write zero-terminators within the buffer. So let's createEntity a writable copy..
    if (ini_size == 0)
